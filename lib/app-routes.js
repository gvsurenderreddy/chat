...
// Load auth mgr lib (chat-sepcific lib).
...
module.exports = function(app) {
  ...
  function requireLogin(req, res, next) {
    ...
    log.info('requireLogin', { request: req });
  ...
  app.post('/login', urlencodedParser, function(req, res) {
    ...
    log.info('GET request on /login received.', { request: req });
  ...
  app.get('/logout', function(req, res) {
    ...
    if (req.session.authToken != null) {
      AuthManager.logOutUser(req.session.authToken, function(err, message) {
       ...
       // Always check err before doing anything!
        if (err) {
         log.error('Error occurred while logging out user.', err);
         return;
        }
  ...
  app.post('/signin', urlencodedParser, function(req, res) {
    ...
    if (req.body.login != '' && req.body.password != '') {
      ...
      AuthManager.createUser(req.body.login, req.body.password, extras, function(err, message) {
        if (err) {
         log.error('Error occurred while creating user.', err);
         res.render(config.templatesUrls.signIn, {
          error: err,
         });
         return;
  ...
  app.get('/userProfile', requireLogin, function(req, res, next) {
    ...
    AuthManager.getUserProfile(req.session.authToken, function(err, extras) {
      if (err) {
        log.error('Error occurred while getting user profile.', err);
  ...
  app.post('/userProfile', function(req, res) {
    ...
    form.parse(req, function(err, fields, files) {
      if (err) {
        log.error('Error occurred while parsing form.', err);
        res.setHeader('Content-Type', 'application/json');
    ...
    form.on('end', function() {
      ...
      AuthManager.setUserProfile(req.session.authToken, extras, function(err, message) {
       ...
       if (err) {
           log.error('Error occurred while setting user profile.', err);
         }
    ...
    form.on('error', function(err) {
      log.error('Error occurred while processing form.', err);
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({
        place: 'form.on(\'error\')',
        error: err,
  ...
  app.use(function(req, res, next) {
    ...
    log.error('Page not found... redirecting to / page', { request: req });
